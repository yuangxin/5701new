...
class PlagiarismPipeline:
    ...
    def write_reports(
        ...
        try:
            ...
            # 不要因为生成 Word 报告失败而中断主流程
            print(f"Warning: failed to write Word report: {e}")

        # 生成段落级 Word 报告（如果有）
        if para_stats and para_details:
            try:
                write_word_report(output_dir / "plagiarism_paragraph_report.docx", para_stats, para_details)
            except Exception as e:
                print(f"Warning: failed to write paragraph Word report: {e}")